...
def integrate(
    ...
    # 注意这里的深度，可能会有多个点投影到一个(u,v)，但是由于zc(u,v,1)=K(xc,yc,zc)即一个像素可能对应相机坐标系下不同深度的多个点
    pix_z = cam_c[:, 2]
    # project all the voxels back to image plane
    # 当前标准空间中的volume即可见区域投影对应的(u,v)坐标，这部分同时完成了相机->图像坐标(以左上角为原点)，公式见https://blog.csdn.net/qq_15029743/article/details/90215104
    pix_x = torch.round((cam_c[:, 0] * fx / cam_c[:, 2]) + cx).long()  # [nx*ny*nz]
    pix_y = torch.round((cam_c[:, 1] * fy / cam_c[:, 2]) + cy).long()  # [nx*ny*nz]

    # Eliminate pixels outside view frustum
    # 可见区域需要投影在当前相机fov中因此第一轮valid_vox_k筛选只是筛选掉无法投影到视窗中的voxel，但是可能有一些靠后深度的voxel也是不可见的此时还筛选不出来
    ...
    # Integrate tsdf
    # 计算sdf,depth_val是深度相机采集的，pix_z是计算得到的
    depth_diff = depth_val - pix_z[valid_pix]
    # sdf很大说明距离太远了即这个voxel虽然也投影对应这个(u,v)但是深度上距离当前扫描的点太远，则无需fusion因此筛掉
    ...
    # 此时valid_vox_x才是真正的重叠区域可见的voxel可以fusion
    ...
    # obs_weight是当前观察点的权重值，这里直接简单的赋值为1了更好的做法是和法向相关
    ...
    if color_vol is not None and color_im is not None:
        ...
        # 这里首先由于image一般是二维，而合法的需要更新的voxel是三维的，所以索性这里先用更宽范围的二维pix_x pix_y采样颜色之后再筛
...
class TSDFVolumeTorch:
    ...
    def get_pts_inside(self, pts, margin=0):
        # 这里的采样点坐标是浮点数，要转换到对应的所属voxel，可能会超范围因此要返还合法坐标掩码
    ...
    def get_rays(self, c2w, intrinsics, H, W):
        ...
        # 这里是返还世界坐标系下的视线方向与原点坐标，t实际上就是相机的位置思考一下为什么？(xw,yw,zw)*w2c(先旋转，旋转不改变世界原点位置但是轴方向改变，再平移)
        # 因此平移其实就是相机在世界坐标系的位置，因此注意从相机恢复到世界也要先逆旋转再平移
